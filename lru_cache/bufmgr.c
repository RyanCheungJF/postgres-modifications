...
#define BufferIsPinned(bufnum)                                     \
	(                                                              \
		...
		: BufferIsLocal(bufnum) ? (LocalRefCount[-(bufnum)-1] > 0) \
 ...
 */
bool ReadRecentBuffer(RelFileNode rnode, ForkNumber forkNum, BlockNumber blockNum,
						  ...
						  BufferAccessStrategy strategy, bool permanent)
{
	...
	return ReadBuffer_common(smgr, permanent ? RELPERSISTENCE_PERMANENT : RELPERSISTENCE_UNLOGGED, forkNum, blockNum,
							 mode, strategy, &hit);
}
...
bool BgBufferSync(WritebackContext *wb_context)
{
	...
	/* cs3223 */
	return true; /* hibernate the background writer process */
 ...
 *		a buffer.
 */
void BufferGetTag(Buffer buffer, RelFileNode *rnode, ForkNumber *forknum,
				  BlockNumber *blknum)
{
	BufferDesc *bufHdr;
 ...
 * --------------------------------------------------------------------
 */
void DropRelFileNodeBuffers(SMgrRelation smgr_reln, ForkNumber *forkNum,
 ...
 * --------------------------------------------------------------------
 */
void CreateAndCopyRelationData(RelFileNode src_rnode, RelFileNode dst_rnode,
							   bool permanent)
{
	RelFileNodeBackend rnode;
	char relpersistence;
	...
	relpersistence = permanent ? RELPERSISTENCE_PERMANENT : RELPERSISTENCE_UNLOGGED;
